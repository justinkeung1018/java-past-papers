...
public class NineTailsWeightedGraph {
   ...
   * 
   * <p>flipResult includes the index of the generated configuration and the number of flips.</p>
   *
  ...
  private FlipResult flipConfiguration(int confIndex, int position) {
    ...
    int count = 0;
    if (flipCell(conf, row, column)) {
      count++;
    }
    if (flipCell(conf, row - 1, column)) {
      count++;
    }
    if (flipCell(conf, row + 1, column)) {
      count++;
    }
    if (flipCell(conf, row, column - 1)) {
      count++;
    }
    if (flipCell(conf, row, column + 1)) {
      count++;
  ...
  private boolean flipCell(char[] conf, int row, int col) {
