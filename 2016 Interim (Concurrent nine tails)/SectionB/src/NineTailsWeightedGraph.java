...
public class NineTailsWeightedGraph {
  ...
  private PriorityQueueInterface<WeightedEdge> generateParents(int index) {
    ...
    for (int pos = 0; pos < 9; pos++) {
      if (conf[pos] == 'H') {
        ...
        // The following line in the skeleton is wrong and has been corrected
  ...
  private void constructMinimumSpanningTree() {
    ...
    while (visited.size() < NUM_CONFIGURATIONS) {
      ...
      int minChild = -1, minParent = -1;

      // Finds the next edge to add to the tree
      for (int child = 1; child <= NUM_CONFIGURATIONS; child++) {
        if (visited.contains(child)) {
          continue;
        }
        PriorityQueueInterface<WeightedEdge> edges = confCopy.get(child);

        while (!edges.isEmpty()) {
          WeightedEdge edgeWithMinWeight = edges.peek();
          edges.remove();
          int parent = edgeWithMinWeight.parent;
          if (visited.contains(parent)) {
            int cost = costs[parent] + edgeWithMinWeight.weight;
            if (cost < minCost) {
              minCost = cost;
              minChild = child;
              minParent = parent;
      ...
      visited.add(visited.size() + 1, minChild);
      costs[minChild] = minCost;
      nextMoves[minChild] = minParent;

      confCopy = getConfigurationsCopy(); // Restores the adjacency list since some edges are removed in while loop
    }
