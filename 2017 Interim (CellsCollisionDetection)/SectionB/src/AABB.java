...
public class AABB {
  ...
  /*
   * Determines whether the region intersects with another region.
   * @param that The other region.
   */
  public boolean intersects(AABB that) {
    if (that == null) {
      return false;
    }
    return left <= that.right && right >= that.left && bottom <= that.top && top >= that.bottom;
  }

  @Override
  public String toString() {
    return "Top left: (" + top + ", " + left + "), bottom right: (" + right + ", " + bottom + ")";
  }
}
